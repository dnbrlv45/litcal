            ...
            {dayEvents.map((ev) => {
              ...
              return (
                <div
                  ...
                  className={`absolute z-10 cursor-pointer overflow-hidden rounded-lg border px-3 py-2 text-xs shadow-sm ring-1 transition hover:-translate-y-0.5 hover:shadow-md ${colors.bg} ${colors.text} ${ev.hasConflict ? "border-amber-400 ring-amber-200" : `${colors.border} ${colors.ring}`}`}
                  style={{
                    top: eventTop(ev) - HOURS[0] * ROW_HEIGHT,
                    height: eventHeight(ev),
                    left: `calc(${ev.col * colWidth}% + 4px)`,
